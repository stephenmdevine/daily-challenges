function gap(g, m, n) { //g is gap size, m is search start, n is search end
    let a = 0;
    let numGap = [];
    for (let i = m; i <= n-g; i++) {
        if (isPrime(i)) {
            numGap[0] = i;
            for (let j = i+2; j <= i+g; j++) {
                if (isPrime(j)) {
                    if (j-numGap[0] == g) {
                        numGap[1] = j;
                        return numGap;
                    }   else if (j-numGap[0] != g) {
                        numGap[0] = j;
                    }
                }
            }
...
/*
The prime numbers are not regularly spaced. For example from 2 to 3 the gap is 1. From 3 to 5 the 
gap is 2. From 7 to 11 it is 4. Between 2 and 50 we have the following pairs of 2-gaps primes: 3-5, 
...
limits m, n if these numbers exist otherwise `nil or null or None or Nothing (or ... depending on the 
language).
...
is not a 6-gap because there is 103 in between and 103-109 is not a 6-gap because there is 107 in 
between.
